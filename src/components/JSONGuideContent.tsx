    ...
    <div className="space-y-12">
        ...
        {guide.sample && (
            <section>
                ...
                <p>This is what is loaded in the editor above.</p>
                <div className="not-prose grid gap-4 lg:grid-cols-2 my-8">
                    <CodePanel label="Pasted" code={guide.sample.input} tone="muted" />
                    <CodePanel label="Result" code={guide.sample.output} tone="primary" />
                </div>
        ...
        {guide.invalidExamples && (
            <section>
                <h2 className="text-3xl font-bold mb-6">Invalid JSON, and exactly why</h2>
                <p>
                    Each of these fails to parse. The error text is what a V8-based engine (Chrome,
                    Edge, Node.js) actually throws for that input — verified by running it, not
...
function RelatedJsonGuides({ current }: { current: string }) {
    ...
    return (
        <section>
            <h2 className="text-3xl font-bold mb-6">Other JSON tools</h2>
            ...
            <p className="mt-6">
                Need the full set of options — sorting, indentation, table and tree views?{' '}
                <Link to="/json" className="text-primary hover:underline">
                    The general JSON formatter
                </Link>{' '}
